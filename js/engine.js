...
var Engine = (function(global) {
        ...
        for (row = 0; row < numRows; row++) {
            for (col = 0; col < numCols; col++) {
                ...
                ctx.drawImage(
                    ...
                    col * layoutSize.width,
                    row * layoutSize.mainPart
                );
            }
        }
     ...
     */
    function reset(type) {
        if (type === 'gameOver') {  // 游戏结束
            // 遮罩层
			ctx.fillStyle = 'rgba(0,0,0,.6)';
			ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height - 20);

			var centerWt = ctx.canvas.width / 2,
				middleHt = ctx.canvas.height / 2;

            // 游戏结束文案
			ctx.font = 'bold 60px Verdana';
			ctx.textAlign = 'center';
			ctx.fillStyle = '#fff';
			ctx.fillText('Game Over', centerWt, middleHt - 60);

			ctx.font = 'normal 30px Consolas';
			ctx.fillText('Score:' + player.score, centerWt, middleHt);
			...
			ctx.fillStyle = '#ff9f21';
			ctx.fillText(text, centerWt, middleHt + 30);

			// 提示信息
			ctx.fillStyle = '#fff';
			ctx.fillText('Press ENTER to retry', centerWt, ctx.canvas.height - 100);

			document.addEventListener('keyup', pressEnter);  // 监听回车键按下
		}
	}
    ...
    Resources.load(
        (function() {
            ...
            for (var key in imgAsset) {
                if (imgAsset.hasOwnProperty(key)) {
                    if (Array.isArray(imgAsset[key])) {  // 数组型
                        imgAsset[key].forEach(function(item) {
                            arr.push(item);
                        });
                    } else {
                        arr.push(imgAsset[key]);
                    }
                }
            }
